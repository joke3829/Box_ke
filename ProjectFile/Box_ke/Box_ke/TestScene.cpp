	...
	case WM_KEYDOWN:
		...
		case '2':
			if (m_Hero) {
				...
				if (AnimName == "RollRight_On") {
					static_cast<CHeroObjectDX11*>(m_Hero.get())->GetAnimPlayer()->Play("RollRight_Off", false);
				}
				else {
					static_cast<CHeroObjectDX11*>(m_Hero.get())->GetAnimPlayer()->Play("RollRight_On", false);
